...
pub fn process_new_project(new_project: &NewProject) {
    ...
    if let Err(e) = start_or_attach_tmux_session(
...
pub fn create_project_dir(new_project: &NewProject) -> Result<String, io::Error> {
    ...
    let relative_project_dir = format!(
        "{}/{}/{}/{}",
        BASE_DIR, project_type, project_language, project_name
    );
...
pub fn process_project_clone(new_clone: &CloneProject) -> Result<(), io::Error> {
    let relativ_path = format!(
        "{}/{}/{}",
        BASE_DIR, new_clone.project_type, new_clone.language
    );
    ...
    println!(
        ...
        new_clone.url, project_dir, project_name
    );
    start_or_attach_tmux_session(
        &project_name,
...
pub fn initialize_git_repo(project_dir: &str) -> Result<(), std::io::Error> {
    Command::new("git").args(["init", project_dir]).status()?;
    // println!("Initialized a new git repository in {}", project_dir);
    Ok(())
}

pub fn start_or_attach_tmux_session(
    project_name: &str,
    project_dir: &str,
    language: &Language,
) -> Result<(), io::Error> {
